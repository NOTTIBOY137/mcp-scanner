...
export function ClaimServerFlow({ serverId }: { serverId: string }) {
  ...
  if (step === "token") {
    return (
      <div className="mt-4 space-y-4 rounded-lg border border-[var(--card-border)] bg-[var(--bg-secondary)]/30 p-4">
        <h3 className="text-sm font-semibold">Claim Verification</h3>
        <ol className="space-y-3 text-sm text-[var(--text-primary)]">
          <li>
            <span className="font-medium text-[var(--text-primary)]">Step 1:</span> Add a
            file called{" "}
        ...
        <div className="flex items-center gap-2">
          <code className="flex-1 rounded bg-[var(--bg-primary)] px-3 py-2 text-sm font-mono text-[var(--text-primary)] break-all">
            {token}
          </code>
          <button
            onClick={copyToken}
            className="shrink-0 rounded-md bg-[var(--bg-tertiary)] px-3 py-2 text-xs font-medium text-[var(--text-primary)] hover:brightness-110 transition-colors"
          >
            {copied ? "Copied!" : "Copy"}
          </button>
        </div>
        <ol start={2} className="space-y-3 text-sm text-[var(--text-primary)]">
          <li>
            <span className="font-medium text-[var(--text-primary)]">Step 2:</span> Commit
            and push the file to your repository.
          </li>
          <li>
            <span className="font-medium text-[var(--text-primary)]">Step 3:</span> Click
            &quot;Verify&quot; below.
          </li>
        </ol>
        <button
          onClick={verify}
          className="rounded-md bg-[var(--accent)] px-4 py-2 text-sm font-medium text-white hover:brightness-110 transition-colors"
        >
          Verify
        </button>
      </div>
