...
const severityConfig: Record<string, { style: string; icon: typeof AlertTriangle }> = {
  ...
  info: { style: "bg-[var(--bg-tertiary)]/15 text-[var(--text-secondary)] border-[var(--card-border)]", icon: Info },
};
    ...
    <div className="card space-y-3">
        ...
        <div className="overflow-hidden rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-primary)]">
          {finding.filePath && (
            <div className="flex items-center gap-2 border-b border-[var(--border-subtle)] px-3 py-2">
              <FileCode className="h-3.5 w-3.5 text-[var(--text-tertiary)]" />
          ...
          {finding.snippet && (
            <pre className="overflow-x-auto p-3 font-mono text-xs text-[var(--text-secondary)] leading-relaxed">
              {finding.snippet}
            </pre>
          )}
        </div>
      ...
      {finding.remediation && (
        <details className="group">
          ...
          <p className="mt-2 text-sm text-[var(--text-secondary)] border-l-2 border-emerald-700/50 pl-3">
            {finding.remediation}
          </p>
        </details>
      )}
