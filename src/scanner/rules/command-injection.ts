...
export const commandInjectionRules: ScanRule[] = [
  ...
  },
  {
    id: "CI009",
    category: "command-injection",
    title: "Go os/exec with string concatenation",
    description: "Using Go's os/exec.Command with concatenated strings enables shell injection.",
    severity: "critical",
    pattern: /exec\.Command\s*\(\s*(?:"[^"]*"\s*\+|fmt\.Sprintf)/g,
    fileFilter: (path: string) => /\.go$/.test(path),
    remediation: "Pass arguments as separate parameters to exec.Command instead of concatenating.",
    owaspMcpTop10: "MCP06",
    cweIds: ["CWE-78"],
  },
  {
    id: "CI010",
    category: "command-injection",
    title: "Rust std::process::Command with unsanitized args",
    description: "Using Rust's Command::new with format! macro or string concatenation for arguments.",
    severity: "high",
    pattern: /Command::new\s*\(\s*(?:format!|&format!|\w+\s*\+)/g,
    fileFilter: (path: string) => /\.rs$/.test(path),
    remediation: "Use Command::arg() with validated literals instead of formatted strings.",
    owaspMcpTop10: "MCP06",
    cweIds: ["CWE-78"],
  },
  {
    id: "CI011",
    category: "command-injection",
    title: "Python os.popen() usage",
    description: "os.popen() passes commands through the shell and is vulnerable to injection.",
    severity: "critical",
    pattern: /\bos\.popen\s*\(/g,
    fileFilter: (path: string) => /\.(py|pyw)$/.test(path),
    remediation: "Replace os.popen() with subprocess.run() using a list of arguments.",
    owaspMcpTop10: "MCP06",
    cweIds: ["CWE-78"],
  },
  {
    id: "CI012",
    category: "command-injection",
    title: "Template literal in shell command construction",
    description: "Building shell commands with template literals allows injection via interpolated values.",
    severity: "high",
    pattern: /(?:exec|execSync|system|popen)\s*\(\s*`[^`]*\$\{/g,
    remediation: "Use parameterized command execution instead of template literals.",
    owaspMcpTop10: "MCP06",
    cweIds: ["CWE-78"],
  },
  {
    id: "CI013",
    category: "command-injection",
    title: "vm.runInNewContext or vm.runInThisContext usage",
    description: "Node.js vm module can execute arbitrary code and is not a security sandbox.",
    severity: "critical",
    pattern: /\bvm\.(?:runInNewContext|runInThisContext|createContext|compileFunction)\s*\(/g,
    remediation: "Avoid vm module for untrusted code. Use a proper sandbox like isolated-vm.",
    owaspMcpTop10: "MCP06",
    cweIds: ["CWE-94"],
  },
  {
    id: "CI014",
    category: "command-injection",
    title: "child_process.execFile with shell option",
    description: "execFile with shell:true negates its safety benefits and enables injection.",
    severity: "high",
    pattern: /\bexecFile(?:Sync)?\s*\([^)]*shell\s*:\s*true/g,
    remediation: "Remove shell: true from execFile. Use execFile with explicit argument arrays.",
    owaspMcpTop10: "MCP06",
    cweIds: ["CWE-78"],
  },
];
